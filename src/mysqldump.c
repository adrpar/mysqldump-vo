...
**
** Updated by Adrian M. Partl (eScience AIP) 2013 to support basic
** VOTable support 
**
*/
...
void swapByteOrder32(unsigned long* ul);
void swapByteOrder64(unsigned long long* ull);
...
#include <sslopt-longopts.h>
  ...
  {"xml", 'X', "Dump a database as well formed XML.", 0, 0, 0, GET_NO_ARG,
   NO_ARG, 0, 0, 0, 0, 0, 0},
  {"vo", OPT_VOTABLE_TABLEDATA, "Dump a database into a simple VO Table format.", 0, 0, 0, GET_NO_ARG,
   NO_ARG, 0, 0, 0, 0, 0, 0},
  {"vo-bin", OPT_VOTABLE_BINARY, "Dump a database into a simple VO Table format using binary representation.", 0, 0, 0, GET_NO_ARG,
   NO_ARG, 0, 0, 0, 0, 0, 0},
  {"vo-bin2", OPT_VOTABLE_BINARY2, "Dump a database into a simple VO Table format using binary2 representation.", 0, 0, 0, GET_NO_ARG,
   NO_ARG, 0, 0, 0, 0, 0, 0},
...
static void write_header(FILE *sql_file, char *db_name)
{
  ...
  else if (opt_vo)
  {
    ...
    /*
      ...
      xsi:type to define an element's data type.
    */
    fputs("<VOTABLE ", sql_file);
    fputs("version=\"1.3\" ", sql_file);
    fputs("xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" ",
          sql_file);
    fputs("xmlns=\"http://www.ivoa.net/xml/VOTable/v1.3\" ",
          sql_file);
    fputs("xmlns:stc=\"http://www.ivoa.net/xml/STC/v1.30\" ",
          sql_file);
    fputs(">\n", sql_file);
    check_io(sql_file);
  }
...
#include <sslopt-case.h>
  ...
  case 'X':
    ...
    break;
  case OPT_VOTABLE_TABLEDATA:
    opt_vo= 1;
    opt_voBin = 0;
    extended_insert= opt_drop= opt_lock=
      opt_disable_keys= opt_autocommit= opt_create_db= 0;
    break;
  case OPT_VOTABLE_BINARY: {
    opt_vo= 1;
    opt_voBin = 1;

    //check if this system is bigendian.
    //TODO: implement little endian handling
    int a = 1;
    if(*(char *)&a = 1) {
      change_endian = 1;
    }
...
/*
  Print VOTable tag with many attributes (translated).
...
static void print_vo_row(FILE *xml_file, const char *row_name,
                          MYSQL_RES *tableRes, MYSQL_ROW *row,
                          const char *str_create, int currRow)
{
    ...
    if ((*row)[i])
    {
      ...
      else
      {
        ...
        if (strncmp(field->name, "Field" , my_min(field->name_length, 5)) == 0) {
          ...
          fputs("=\"", xml_file);
          typeStr = getVOTableType((*row)[i], lengths[i]);
          
          if(typeStr != NULL) {
            print_quoted_xml(xml_file, typeStr, strlen(typeStr), 0);
            fputc('"', xml_file);

            //add time properties
            handleDATETIMEVOTableTypes(xml_file, (*row)[i], lengths[i]);
          } else {
            //handle special cases where more information needs to be given:

            //DEC, NUMERIC, FIXED
            if(handleDECVOTableTypes(xml_file, (*row)[i], lengths[i]) == 0) {
              //STRINGS
              if(handleCHARVOTableTypes(xml_file, (*row)[i], lengths[i]) == 0) {
                char tmp[256];
                strncpy(tmp, (*row)[i], lengths[i]);
                tmp[lengths[i]] = '\0';

                fprintf(stderr, "%s type is currently unsupported by mysqldump VOTables format\n", tmp);
  ...
  if (create_stmt_len)
  {
    ...
    print_xml_cdata(xml_file, create_stmt_ptr, create_stmt_len);
    fprintf(xml_file, "\t\t</%s>\n", row_name);
  } else if (canBeNull == TRUE && opt_voBin == 1 && typeStr != NULL) {
    //obtain MAX_INT value as string, depending on the datatype
    char nullValueStr[100];
    switch(typeStr[0]) {
      case 'b':
        //bool or bit
        if(typeStr[1] == 'o') {
          sprintf(nullValueStr, "%i", CHAR_MAX);
        } else {
          fputs(" />\n", xml_file);
          check_io(xml_file);
          return;
      ...
      case 'u':
        ...
        break;
      case 's':
        sprintf(nullValueStr, "%i", SHRT_MAX);
        break;
      case 'i':
        sprintf(nullValueStr, "%li", INT_MAX32);
        break;
      case 'l':
        sprintf(nullValueStr, "%lli", LLONG_MAX);
        break;
      default:
          fputs(" />\n", xml_file);
          check_io(xml_file);
    ...
    fputs(">\n", xml_file);
    fputs("\t\t\t<VALUES null=\"", xml_file);
    fputs(nullValueStr, xml_file);
    fputs("\" />\n", xml_file);
    fputs("\t\t</FIELD>\n", xml_file);
  }
  else
                                ...
                                char *ignore_flag)
{
  ...
  *ignore_flag= check_if_ignore_table(table, table_type);
  ...
  else
  {
    ...
    int numCols = mysql_num_rows(result);
    fieldLenArray = (int*)my_malloc(numCols * sizeof(int), MYF(0));
    if(fieldLenArray == NULL) {
      die(EX_MYSQLERR, "Unable to allocate memory\n");
    }
    ...
    if (opt_vo) {
      ...
      if(opt_voBin) {
        ...
        print_xml_tag(md_result_file, "\t", "\n", "STREAM", "encoding=", "base64",
                NullS);
/*        print_xml_tag(md_result_file, "\t", "", "BINARY", NULL, NULL,
                NullS);*/
      } else {
        print_xml_tag(md_result_file, "\t", "\n", "TABLEDATA", NULL, NULL,
                NullS);
    ...
    //variables for VOTable binary representation
    char* binRepr;
    int lenBinRepr;
    int currPosBinRepr = 0;
    int numFields = mysql_num_fields(res);
    int lenNullField = (numFields + 7) / 8;
    char nullField[lenNullField];
    char *strBuffer;
    int lenBuffer = 1024;
    int lenWritten = 0;
    char base64Overflow[3]; //holding the 3 bytes needed for joining two rows in base64
    int lenBase64Overflow = 0;

    if (opt_voBin) {
      if(opt_voBin == 1)
        lenNullField = 0;

      memset(&nullField, 0, lenNullField);
      binRepr = (char*)my_malloc(lenNullField + lenBuffer, MYF(0));
      lenBinRepr = lenNullField + lenBuffer;
      currPosBinRepr = lenNullField;
      strBuffer = (char*)my_malloc(lenBuffer, MYF(0));
      if(strBuffer == NULL || binRepr == NULL) {
        die(EX_MYSQLERR, "Couldn't allocate memory");          
      }
      memset(binRepr, 0, lenNullField);
    }
    ...
    while ((row= mysql_fetch_row(res)))
    {
      ...
      else if (opt_voBin)
      {
        ...
        //convert to base64
        size_t needed_length= base64_needed_encoded_length(currPosBinRepr - lenBase64Overflow);
        char* tmp = (char*) my_malloc(needed_length, MYF(0));
        if(tmp == NULL) {
          die(EX_MYSQLERR, "Couldn't allocate memory");
        }

        if(base64_encode(binRepr, currPosBinRepr - lenBase64Overflow, tmp) != 0) {
        ...
        if(currPosBinRepr > lenBuffer) {
          binRepr = my_realloc(binRepr, lenBuffer, MYF(0));
          lenBinRepr = lenBuffer;
        }

        if(lenBase64Overflow > 0) {
          //copy the remaining bytes over to the new buffer
          memcpy(binRepr, (void*)(binRepr + currPosBinRepr - lenBase64Overflow), lenBase64Overflow);
        }

        currPosBinRepr = lenBase64Overflow + lenNullField;
        memset((char*)(binRepr + lenBase64Overflow), 0, lenNullField);
      }
      else if (opt_vo)
      {
        fputs("\t</TR>\n", md_result_file);
    ...
    if (opt_vo) {
      if(opt_voBin) {
        //write the remaining stuff from the base64 conversion to the file
        if(lenBase64Overflow > 0) {
          //convert to base64
          size_t needed_length= base64_needed_encoded_length(lenBase64Overflow);
          char* tmp = (char*) my_malloc(needed_length, MYF(0));
          if(tmp == NULL) {
            die(EX_MYSQLERR, "Couldn't allocate memory");
          }

          if(base64_encode(binRepr, lenBase64Overflow, tmp) != 0) {
            die(EX_MYSQLERR, "Error in base64 conversion.");
          }
...
/*
  The following are additional functions needed by VOTables, that are not
  needed in the XML implementation
*/
...
const char *getVOTableType(char *typeStr, int len) {
  if (strncmp(typeStr, "bool", my_min(len, 4)) == 0) {
    ...
    if(len > 7) {
      ...
      int charLen = 0;
      sscanf(typeStr, "float(%i)", &charLen);
...
int handleDATETIMEVOTableTypes(FILE *xml_file, char *typeStr, int len) {
  ...
  if (strncmp(typeStr, "time", my_min(len, 4)) == 0 && len == 4) {
    ...
    fputs("=\"", xml_file);
    print_quoted_xml(xml_file, "mysql:TIME", strlen("mysql:TIME"), 0);
    fputc('"', xml_file);
  } else {
    fputc(' ', xml_file);
    print_quoted_xml(xml_file, "xtype", 5, 1);
    fputs("=\"", xml_file);
    print_quoted_xml(xml_file, "adql:TIMESTAMP", strlen("adql:TIMESTAMP"), 0);
    fputc('"', xml_file);
  }

  fputc(' ', xml_file);
  print_quoted_xml(xml_file, "arraysize", 9, 1);
  fputs("=\"", xml_file);
  print_quoted_xml(xml_file, "25*", strlen("25*"), 0);
  fputc('"', xml_file);
...
int handleDECVOTableTypes(FILE *xml_file, char *typeStr, int len) {
  ...
  int m, d = 0;
  char *substr;
  substr = strstr(typeStr, "(");
  sscanf(substr, "(%i,%i)", &m, &d);

  //project this data type to double
  //datatype property tag already printed outside this function
  print_quoted_xml(xml_file, VOTable_types_names[9], strlen(VOTable_types_names[9]), 0);
  fputc('"', xml_file);
  ...
  if(m > 0) {
    ...
    fputc(' ', xml_file);
    print_quoted_xml(xml_file, "width", 5, 1);
    fputs("=\"", xml_file);
    print_quoted_xml(xml_file, tmp, strlen(tmp), 0);
    fputc('"', xml_file);
  }
  ...
  if(d > 0) {
    ...
    fputc(' ', xml_file);
    print_quoted_xml(xml_file, "precision", 9, 1);
    fputs("=\"", xml_file);
    print_quoted_xml(xml_file, tmp, strlen(tmp), 0);
    fputc('"', xml_file);
  }  
...
int handleCHARVOTableTypes(FILE *xml_file, char *typeStr, int len) {
  ...
  //datatype property tag already printed outside this function
  print_quoted_xml(xml_file, VOTable_types_names[6], strlen(VOTable_types_names[6]), 0);
  fputc('"', xml_file);
...
void voTableBinRegisterNull(char* nullField, int len, my_bool isNull) {
  //first shift everything by one bit to the left, starting with most significant end
  int i;
  for(i=0; i < len; i++) {
    nullField[i] = nullField[i] << 1;

    //if this is not the last byte, take the most sign. bit of the next element over to this
    //byte
    if(i != len - 1) {
      char val = 0;
...
//returns buffer length - if buffer to small, this will increase it...
int voTableBinCastFieldToBinary(char *buffer, int lenBuff, char *value, int len, MYSQL_FIELD *field, int *lenWritten, int lenField) {
  switch(field->type) {
    case MYSQL_TYPE_BIT: 
    case MYSQL_TYPE_TINY: {
      if(lenBuff < (1 + sizeof(char)) * sizeof(char)) {
        lenBuff = (1 + sizeof(char)) * sizeof(char);
        buffer = (char*)my_realloc(buffer, lenBuff, MYF(0));
        if(buffer == NULL) {
          die(EX_MYSQLERR, "Couldn't allocate memory in voTableBinCastFieldToBinary");
        }
      }
      ...
      *lenWritten = sizeof(char);      }
      break;
    case MYSQL_TYPE_SHORT: {
      if(lenBuff < (1 + sizeof(short)) * sizeof(char)) {
        lenBuff = (1 + sizeof(short)) * sizeof(char);
        buffer = (char*)my_realloc(buffer, lenBuff, MYF(0));
        if(buffer == NULL) {
          die(EX_MYSQLERR, "Couldn't allocate memory in voTableBinCastFieldToBinary");
        }
      ...
      if(change_endian == 1)
        swapByteOrder16((unsigned short*)&tmp);

      *(short*)buffer = (short)tmp;
  
      buffer[sizeof(short)] = '\0';
      *lenWritten = sizeof(short);
      }
      break; 
    case MYSQL_TYPE_INT24: 
    case MYSQL_TYPE_LONG: {
      if(lenBuff < (1 + sizeof(int32_t)) * sizeof(char)) {
        lenBuff = (1 + sizeof(int32_t)) * sizeof(char);
        buffer = (char*)my_realloc(buffer, lenBuff, MYF(0));
        if(buffer == NULL) {
          die(EX_MYSQLERR, "Couldn't allocate memory in voTableBinCastFieldToBinary");
        }
      ...
      if(change_endian == 1)
        swapByteOrder32((unsigned long*)buffer);

      buffer[sizeof(int32_t)] = '\0';
      *lenWritten = sizeof(int32_t);
      }
      break; 
    case MYSQL_TYPE_LONGLONG: {
      if(lenBuff < (1 + sizeof(int64_t)) * sizeof(char)) {
        lenBuff = (1 + sizeof(int64_t)) * sizeof(char);
        buffer = (char*)my_realloc(buffer, lenBuff, MYF(0));
        if(buffer == NULL) {
          die(EX_MYSQLERR, "Couldn't allocate memory in voTableBinCastFieldToBinary");
        }
      ...
      if(change_endian == 1)
        swapByteOrder64((unsigned long long*)buffer);

      buffer[sizeof(int64_t)] = '\0';
      *lenWritten = sizeof(int64_t);
      }
      break; 
    case MYSQL_TYPE_FLOAT: {
      if(lenBuff < (1 + sizeof(float)) * sizeof(char)) {
        lenBuff = (1 + sizeof(float)) * sizeof(char);
        buffer = (char*)my_realloc(buffer, lenBuff, MYF(0));
        if(buffer == NULL) {
          die(EX_MYSQLERR, "Couldn't allocate memory in voTableBinCastFieldToBinary");
        }
    ...
    case MYSQL_TYPE_DECIMAL: {
      if(lenBuff < (1 + sizeof(double)) * sizeof(char)) {
        lenBuff = (1 + sizeof(double)) * sizeof(char);
        buffer = (char*)my_realloc(buffer, lenBuff, MYF(0));
        if(buffer == NULL) {
          die(EX_MYSQLERR, "Couldn't allocate memory in voTableBinCastFieldToBinary");
        }
      ...
      *lenWritten = sizeof(double);
      ...
      break; 
    case MYSQL_TYPE_STRING: {
      if(lenBuff < (1 + lenField) * sizeof(char)) {
        lenBuff = (1 + lenField) * sizeof(char);
        buffer = (char*)my_realloc(buffer, lenBuff, MYF(0));
        if(buffer == NULL) {
          die(EX_MYSQLERR, "Couldn't allocate memory in voTableBinCastFieldToBinary");
        }
      ...
      //fill remaining string with whitespaces
      if(len < lenField) {
        int i;
        for(i=0; i<(lenField-len); i++) {
          buffer[len + i] = '\0';
        }
      }
    ...
    case MYSQL_TYPE_BLOB: {
      //can also be TEXT
      if(lenBuff < (1 + sizeof(int32_t) + len) * sizeof(char)) {
        lenBuff = (1 + sizeof(int32_t) + len) * sizeof(char);
        buffer = (char*)my_realloc(buffer, lenBuff, MYF(0));
        if(buffer == NULL) {
          die(EX_MYSQLERR, "Couldn't allocate memory in voTableBinCastFieldToBinary");
        }
      ...
      if(value == NULL || strlen(value) == 0) {
        ...
        *(int32_t*)(buffer+0) = len;

        if(change_endian == 1)
          swapByteOrder32((unsigned long*)(buffer+0));

        memcpy((char*)(buffer+sizeof(int32_t)), value, len);
      }

      buffer[sizeof(int32_t) + len] = '\0';
      *lenWritten = sizeof(int32_t) + len;
    }
    break;
    default:
      fprintf(stderr, "Mysql internal type %i is currently unsupported by mysqldump VOTables binary format\n", field->type);
      maybe_exit(EXIT_FAILURE);
  }
...
void swapByteOrder16(unsigned short* us)
{
    ...
    *us = ((tmp >> 8) & 0x00FF) | ((tmp << 8) & 0xFF00);
}

void swapByteOrder32(unsigned long* ui)
{
    unsigned long tmp = *ui;
    *ui =((tmp >> 24) & 0x000000FF) |
         ((tmp<<8) & 0x00FF0000) |
         ((tmp>>8) & 0x0000FF00) |
         ((tmp << 24) & 0xFF000000);
}
...
void swapByteOrder64(unsigned long long* ull)
{
    ...
    *ull = ((tmp >> 56) & 0x00000000000000FF) |
          ((tmp<<40) & 0x00FF000000000000) |
          ((tmp<<24) & 0x0000FF0000000000) |
          ((tmp<<8) & 0x000000FF00000000) |
          ((tmp>>8) & 0x00000000FF000000) |
          ((tmp>>24) & 0x0000000000FF0000) |
          ((tmp>>40) & 0x000000000000FF00) |
